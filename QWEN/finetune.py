...
import pandas as pd
import re
from evaluate import extract_emotion_from_text
...
# 更新情感映射，将frustrated相关的都映射到sad
EMOTION_MAPPING = {
    "Neutral state": "neutral",
    "Frustration": "sad",  # 将Frustration映射到sad
    "Anger": "angry",
    "Happiness": "happy",
    "Excited": "happy",
    "Sadness": "sad",
    "neutral": "neutral",
    "frustrated": "sad",  # 将frustrated映射到sad
    "angry": "angry",
    "happy": "happy",
    "sad": "sad",
    # IEMOCAP原始标签
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        try:
            ...
            if self.split == "train":
                # 训练时使用多样的提示，并打乱情感顺序
                prompt_templates = [
                    "Tell me the emotion in this audio. Options: happy, sad, neutral, angry.",
                    "What emotion does this person express? Choose from: neutral, angry, happy, sad.",
                    "Identify the emotion: sad, neutral, happy, angry."
                ]
                prompt_template = random.choice(prompt_templates)
            else:
                # 验证时也使用不同的情感顺序
                prompt_templates = [
                    "What is the emotion in this audio? happy, sad, neutral, angry.",
                    "What is the emotion in this audio? neutral, angry, happy, sad.",
                    "What is the emotion in this audio? sad, neutral, happy, angry."
                ]
                prompt_template = random.choice(prompt_templates)
...
def get_peft_config() -> LoraConfig:
    target_modules = ["q_proj", "k_proj", "v_proj", "o_proj", "out_proj", "lm_head"]
    ...
    for idx, item in enumerate(tqdm(test_dataset.samples, desc="评估进度")):
        try:
            ...
            for prompt in prompts:
                ...
                prediction_text = processor.batch_decode(output, skip_special_tokens=True)[0].strip()
                # 记录原始预测文本
                logger.info(f"原始预测: '{prediction_text}'")

                # 记录input_ids和output的长度差异来检查是否有新生成的token
                input_length = len(inputs["input_ids"][0])
                output_length = len(output[0])
                new_tokens = output_length - input_length
                logger.info(f"输入长度: {input_length}, 输出长度: {output_length}, 新token数: {new_tokens}")

                # 解码新生成的token
                if new_tokens > 0:
                    new_token_ids = output[0][input_length:]
                    decoded_new = processor.tokenizer.decode(new_token_ids)
                    logger.info(f"新生成的token解码: '{decoded_new}'")
                
                emotion = extract_emotion_from_text(prediction_text)
                if emotion:
...
# Enhanced EmotionMetricsCallback class for monitoring training metrics
class EmotionMetricsCallback(TrainerCallback):
    def __init__(self, eval_dataset, processor, iemocap_root=None):
        self.eval_dataset = eval_dataset
        self.processor = processor
        self.iemocap_root = iemocap_root or ""
    ...
    def on_evaluate(self, args, state, control, model, metrics=None, **kwargs):
        self.eval_steps += 1
        logger.info(f"\n=== Evaluation Step {self.eval_steps} ===")
        logger.info("Starting detailed evaluation...")
        
        # 记录当前评估损失
        current_loss = metrics.get("eval_loss", 0) if metrics else 0
        logger.info(f"Current evaluation loss: {current_loss:.4f}")
        
        # 创建一个小样本数据集用于快速评估
        if len(self.eval_dataset.samples) > 50:
            # 随机抽样但保持类别平衡
            samples_by_emotion = {}
            ...
            # 每个类别抽取相同数量的样本
            sample_count = min([len(samples) for samples in samples_by_emotion.values()])
            # 每个类别最多抽取10个样本
            sample_count = min(sample_count, 10)
            
            mini_eval_samples = []
            for emotion, samples in samples_by_emotion.items():
                mini_eval_samples.extend(random.sample(samples, sample_count))
            
            # 创建一个简单的类来存储样本，而不是创建完整的IEMOCAPDataset实例
            class MiniEvalDataset:
                def __init__(self, samples):
                    self.samples = samples
            
            mini_eval_dataset = MiniEvalDataset(mini_eval_samples)
        else:
            # 如果样本数量已经很小，就使用全部样本
            mini_eval_dataset = self.eval_dataset
        
        # 使用evaluate_model函数进行评估
        try:
            accuracy, report, results, metrics_dict = evaluate_model(
                model=model,
                processor=self.processor,
                test_dataset=mini_eval_dataset,
                iemocap_root=self.iemocap_root,
                save_results=False  # 不保存中间评估结果
            )
            
            # 打印主要指标
            logger.info(f"Callback Evaluation - Accuracy: {accuracy:.4f}")
            logger.info(f"Macro F1: {metrics_dict['report']['macro avg']['f1-score']:.4f}")
            logger.info(f"Weighted F1: {metrics_dict['report']['weighted avg']['f1-score']:.4f}")
            ...
            if accuracy > self.best_accuracy:
                ...
                logger.info(f"\n*** New best accuracy: {accuracy:.4f} at step {state.global_step} ***")
                ...
                logger.info(pd.DataFrame(
                    conf_matrix,
                    index=[f'True_{e}' for e in VALID_EMOTIONS],
                    columns=[f'Pred_{e}' for e in VALID_EMOTIONS]
                ))
        
        except Exception as e:
            logger.error(f"Error during callback evaluation: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())

def main():
    logger.info("Starting fine-tuning of Qwen2-Audio model for four-class emotion recognition...")
    ...
    # Apply monkey patch to base_model.forward
    ...
    def new_base_forward(*args, **kwargs):
        ...
        if "attention_mask" in kwargs and "input_ids" in kwargs:
            input_shape = kwargs["input_ids"].shape
            attention_mask_shape = kwargs["attention_mask"].shape
            
            if input_shape != attention_mask_shape:
                # Crop or pad attention mask to match input shape
                if len(input_shape) == 2:  # [batch_size, seq_len]
                    batch_size, seq_len = input_shape
                    kwargs["attention_mask"] = kwargs["attention_mask"][:, :seq_len]
                    # Pad attention mask if it's too short
                    if kwargs["attention_mask"].shape[1] < seq_len:
                        padding = torch.zeros(
                            (kwargs["attention_mask"].shape[0], seq_len - kwargs["attention_mask"].shape[1]),
                            dtype=kwargs["attention_mask"].dtype,
                            device=kwargs["attention_mask"].device
                        )
                        kwargs["attention_mask"] = torch.cat([kwargs["attention_mask"], padding], dim=1)
        
        # Ensure labels shape matches input_ids
        if "labels" in kwargs and "input_ids" in kwargs:
            input_shape = kwargs["input_ids"].shape
            labels_shape = kwargs["labels"].shape
            
            if input_shape != labels_shape:
                # Crop or pad labels to match input shape
                if len(input_shape) == 2:  # [batch_size, seq_len]
                    batch_size, seq_len = input_shape
                    # Crop if labels are too long
                    if labels_shape[1] > seq_len:
                        kwargs["labels"] = kwargs["labels"][:, :seq_len]
                    # Pad with -100 if labels are too short (-100 means ignore position)
                    elif labels_shape[1] < seq_len:
                        padding = torch.full(
                            (labels_shape[0], seq_len - labels_shape[1]),
                            -100,
                            dtype=kwargs["labels"].dtype,
                            device=kwargs["labels"].device
                        )
                        kwargs["labels"] = torch.cat([kwargs["labels"], padding], dim=1)
    ...
    emotion_callback = EmotionMetricsCallback(
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks=[emotion_callback]  # Add the callback
    )
